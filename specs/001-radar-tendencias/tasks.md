...
- [X] T006 **SPIKE DIA 1 — VALIDADO**: `infra/spike_web_search.py` confirmou Web Search tool + `gpt-5-radar` na nova agents API (`AIProjectClient.get_openai_client().responses.create`); 30,3s/pergunta, 13 citações reais (NVIDIA/Siemens/Intel/Accenture/Deloitte), anotação `{url,title,start_index,end_index}`. Resultado em `infra/spike_result.json`, achado registrado em `docs/critical-review.md`. Confirma: 4 perguntas do R2 DEVEM rodar concorrentes (asyncio.gather) para caber em SC-001
- [X] T007 [P] Implement Pydantic models in `src/radar/models.py`: Evidence (snippet ≤500 chars, URL obrigatória), PanelSection, SupportGrade, Report, ReportSummary, enums SourceType/SectionKey/ReportStatus/SupportLevel — conforme data-model.md e report-schema.json
- [X] T008 [P] Implement settings in `src/radar/config.py` (pydantic-settings): PROJECT_ENDPOINT, MODEL_DEPLOYMENT_NAME (default gpt-5-radar), COSMOS_ENDPOINT/KEY, ANALYSIS_BUDGET_SECONDS=360, MAX_ANALYSES_PER_DAY=10, RADAR_OFFLINE flag
- [X] T009 [P] Define `ReportRepository` protocol in `src/radar/storage/base.py` (save, get(id, theme_slug) point read, list_summaries, find_by_slug) per contracts §7
- [X] T010 Implement `LocalReportRepository` in `src/radar/storage/local.py` (JSON files em `data/reports/` — R9; usado também pelos testes) + unit test `tests/unit/test_local_repo.py` (5/5 passando)
- [X] T011 Implement `CosmosReportRepository` in `src/radar/storage/cosmos.py` (point read, projeção leve no list_summaries, retry 1x na escrita) + factory por RADAR_OFFLINE em `src/radar/storage/__init__.py`
- [X] T012 [P] Define `Collector` protocol + `CollectorResult` in `src/radar/collectors/base.py` (nunca propaga exceção, degraded/error, timeout por fonte — contracts §1)
- [X] T013 [P] Create test fixtures in `tests/fixtures/`: arxiv_atom.xml, openalex_response.json, agent_citations.json, synthesizer_output.json (válido e inválido)
...
- [X] T035 [P] Write `infra/provision.md` + provisionar Azure conforme quickstart.md §3: RG (`radar-trends`, brazilsouth — não eastus, ver achado real), Cosmos serverless (radar/reports), App Service B1 (Always On, WebSockets, startup command, App Settings), Managed Identity (role `Foundry User` — **atribuída manualmente pelo usuário via portal**, CLI bloqueado por erro real `MissingSubscription`), budget alert (**criado manualmente pelo usuário via portal** — CLI de `az consumption budget` com bug reproduzível)
- [X] T036 Deploy app no App Service + smoke test real: **os 2 temas do desafio completaram com sucesso** contra Foundry/Cosmos de produção (Edge AI: 123s/53 evidências/4 tipos de fonte; Robôs Humanoides: 37s/44 evidências/3 tipos de fonte; SC-001 e SC-005 PASS nos dois, zero degradação). 3 bugs reais encontrados e corrigidos só nesta etapa (arXiv migrou p/ HTTPS, capacidade do deployment insuficiente 10→200, síntese não tratava RateLimitError). Resultado em `infra/smoke_test_result.json`. **Redeploy do código corrigido para o App Service pendente** — 8 tentativas (`az webapp deploy` + `config-zip`) falharam por instabilidade de rede desta sessão especificamente no endpoint SCM (outras chamadas Azure funcionam normalmente); comando documentado em `infra/provision.md` §5 para rodar de outra rede
...
- [X] T038 [P] Update `docs/critical-review.md`: achados reais de provisionamento (região, WebSockets, atrito de rede), achados do code review + smoke test e correções aplicadas
- [X] T039 Code review pelo agente **octo-code-reviewer** (gate da constituição): veredito "aprovado com ressalvas"; 3 achados ALTO corrigidos (OpenAlex `source=null` derrubava a coleta, `gather` sem `return_exceptions` propagava exceção crua, síntese bloqueante sem timeout efetivo) + 1 MÉDIO (histórico mostrava `running` como completo); 43/43 testes passando, ruff limpo
- [X] T040 Preparar demo (quickstart §4): 4 relatórios gerados e exportados (2 completos + 2 parciais, Edge AI e Robôs Humanoides p/ Indústria), `RADAR_OFFLINE=1` testado localmente com sucesso (health check OK, 4/4 relatórios reabrem). Roteiro de 10 min e liberação do IP nas Access Restrictions ficam para o dia da apresentação (documentado no runbook do portal)
